...
//        collectionView.reloadData()
        ...
        timesViewed = defaults.object(forKey: "count") as? [Int] ?? [Int]()
    }
    
    
    func save() {
        let defaults = UserDefaults.standard
        defaults.set(timesViewed, forKey: "count")
    }
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if !timesViewed.indices.contains(indexPath.row) {
            cell.countLabel.text = "0 times viewed"
            timesViewed.insert(0, at: indexPath.row)
            save()
            collectionView.reloadData()
        } else {
            cell.countLabel.text = "\(timesViewed[indexPath.row] ) times viewed"
        }
